...
const RollEntryModal: React.FC<RollEntryModalProps> = ({ bill, onClose, onSave }) => {
    ...
    const fetchRolls = async () => {
        setLoading(true);
        setRolls([]); // Reset new rolls when bill changes
        setSessionCounter(0); // Reset session counter
        try {
            // 1. Fetch next available global sequence FIRST (High Priority)
            ...
            // 2. Fetch existing rolls
            const result = await purchaseApi.getRolls(bill.id);
            if (result.error) throw new Error(result.error);
            setExistingRolls(result.data || []);

            // 3. Check pending quantity for all materials in bill (Slow process)
            if (bill.supplier?.id) {
                let totalPending = 0;
            ...
            // Only set fallback if we don't have a sequence yet
            setNextSeq(prev => prev === null ? 1 : prev);
        }
        setLoading(false);
    };

    // Auto-correct roll codes once nextSeq is loaded
    useEffect(() => {
        if (nextSeq !== null && rolls.length > 0) {
            setRolls(prevRolls => {
                let changed = false;
                const updated = prevRolls.map((r, idx) => {
                    if (r.isAutoGenerated) {
                        const currentSeq = nextSeq + idx;
                        const newCode = `ROLL-${String(currentSeq).padStart(4, '0')}`;
                        if (r.rollCode !== newCode) {
                            changed = true;
                            return { ...r, rollCode: newCode };
                        }
                    }
                    return r;
                });
                return changed ? updated : prevRolls;
            });
            // Sync session counter to ensure NEXT added roll uses correct sequence
            setSessionCounter(rolls.length);
        }
    }, [nextSeq, rolls.length]);
    ...
    const handleAddRow = (materialId?: string) => {
        ...
        // Use a strictly increasing session counter to avoid duplicates if rows are deleted
        const currentSeq = (nextSeq || 0) + sessionCounter;
        const newRollCode = nextSeq === null ? 'Loading...' : `ROLL-${String(currentSeq).padStart(4, '0')}`;

        console.log(`🎯 Generated Roll Code: ${newRollCode} (nextSeq=${nextSeq}, sessionCounter=${sessionCounter})`);
    ...
    const handleUpdateRow = (id: string, field: keyof RollItem, value: string) => {
        setRolls(rolls.map(r => {
            if (r.id === id) {
                ...
                // If user manually edits the roll code, stop auto-generation for this row
                if (field === 'rollCode') {
    ...
    const handleKeyDown = (e: React.KeyboardEvent, rowIndex: number, field: string) => {
        if (e.key === 'Enter') {
            ...
            if (rowIndex === rolls.length - 1) {
                // Inherit material from previous row
                            ...
                            <div className="flex gap-2">
                                {billItems.length > 1 ? (
                                    ...
                                    <button
                                        ...
                                        disabled={loading || nextSeq === null}
                                        className={`flex items-center gap-1 text-xs px-4 py-2 rounded-lg transition-colors font-bold shadow-sm ${loading || nextSeq === null ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                                        autoFocus
                                    >
